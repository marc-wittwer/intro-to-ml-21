...
clf = MLPClassifier(hidden_layer_sizes=(10,5), activation="relu", solver="adam", alpha=0.001, max_iter=10000)
...
submission5= pandas.DataFrame(columns=['Id', 'y'])
i=0
for x in y_pred:
    submission5.loc[i] = [str(i+2000),(y_pred[i])]
    i = i + 1
df = pandas.DataFrame(["Id", "y"])
submission5.append(df)

submission5.to_csv("submission5.csv", index=False, header=True)
