import pandas as pd
import numpy as np
from sklearn.preprocessing import StandardScaler
from sklearn import svm
from sklearn.pipeline import Pipeline
from sklearn.impute import SimpleImputer
from sklearn.model_selection import GridSearchCV
from sklearn.feature_selection import SelectKBest, mutual_info_classif
from sklearn.linear_model import Lasso

n_jobs = 3  # number of CPU thresds to use
np.random.seed(42)  # global random seed to get reproducible results

# Load already extracted features. Generated by feature_extraction.py from original data
train_features = pd.read_csv('data/train_extracted_features.csv', index_col='pid')
train_labels = pd.read_csv('data/train_extracted_labels.csv', index_col='pid')
test_features = pd.read_csv('data/test_extracted_features.csv')


# Subtask 1 & 2
medical_tests = ['LABEL_BaseExcess', 'LABEL_Fibrinogen', 'LABEL_AST',
                'LABEL_Alkalinephos', 'LABEL_Bilirubin_total', 'LABEL_Lactate',
                'LABEL_TroponinI', 'LABEL_SaO2', 'LABEL_Bilirubin_direct',
                'LABEL_EtCO2', 'LABEL_Sepsis']  # Columns with test labels

grid_search_params = {'svc__C': [0.1, 0.15, 0.2, 0.9, 1, 1.1],
                      'imputer__strategy': ['mean', 'constant']}  # Params for CV: Regularization of svc and imputation strategy

# fit a classificator for each test
for test_label in medical_tests:
    pipeline = Pipeline(steps=[('imputer', SimpleImputer(fill_value=0)),            # Impute nan values
                               ('scaler', StandardScaler()),                        # Standardization
                               ('sel', SelectKBest(mutual_info_classif, k=100)),    # Feature Selection
                               ('svc', svm.SVC(class_weight='balanced'))])          # SVM for classification

    # Crossvalidation using roc_auc score and our pipeline
    clf = GridSearchCV(pipeline, param_grid=grid_search_params, scoring='roc_auc', n_jobs=n_jobs, verbose=3, cv=None)
    clf.fit(train_features, train_labels[test_label])

    # Save and print results
    results_df = pd.DataFrame({'param': clf.cv_results_["params"], 'mean_score': clf.cv_results_["mean_test_score"],
                               'fit_time': clf.cv_results_["mean_fit_time"]})
    print(results_df)


# Subtask 3
vital_signs = ['LABEL_RRate', 'LABEL_ABPm', 'LABEL_SpO2', 'LABEL_Heartrate']   # Columns with labels
grid_search_params_3 = {'reg__alpha': [0.1, 0.11, 0.32, 1, 1.2, 2]}  # Params for CV: Lasso Regularization strength
results = []

# fit regressors
for sign in vital_signs:
    print("fitting " + sign)
    pipeline_3 = Pipeline(steps=[('imputer', SimpleImputer(strategy='mean')),   # Impute nan values
                                 ('scaler', StandardScaler()),                  # Standardization
                                 ('reg', Lasso())])                             # Lasso regression

    # Crossvalidation using r2 score and our pipeline
    model = GridSearchCV(pipeline_3, param_grid=grid_search_params_3, scoring='r2', n_jobs=n_jobs, verbose=3, cv=None)
    model.fit(train_features, train_labels[sign])

    # Save results
    results.append(pd.DataFrame({'param': model.cv_results_["params"],
                                 'mean_score': model.cv_results_["mean_test_score"],
                                 'fit_time': model.cv_results_["mean_fit_time"]}))

# Print results for evaluation
for i, result in enumerate(results):
    print("\n --- " + vital_signs[i] + " ---")
    print(result)
